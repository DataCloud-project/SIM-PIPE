...
<div class="{isOpen ? 'block' : 'hidden'} fixed z-10 inset-0 overflow-y-auto variant-ghost">
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <!-- Modal Panel -->
    <div class="inline-block align-bottom rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle w-4/5 p-4 variant-filled-surface mx-auto">
      <div>
      ...
      <div>
        ...
        <div>
          <div>
            <button type="button" class="btn variant-filled-ghost float-left" on:click={cancel}>Cancel</button>
          </div>
          <div>
            <button 
              type="button" 
              ...
              on:click={submit}>Submit selected artifact
            </button>
          </div>
        </div>
      </div>
