    ...
    onMount(() => {
        ...
        if (regressionType === 'power') {
            ...
            for (let i = 0; i < numberOfPoints; i++) {
                ...
                const yValue = regressionType === 'power' ? coeffs[0] * xValue ** coeffs[1] : coeffs[0] * xValue + coeffs[1];
                lineData.push({ x: xValue, y: yValue });
            }

            // generate the last point (max x, max y)
            lineData.push({ x: d3.max(data, d => d.x), y: coeffs[0] * d3.max(data, d => d.x) ** coeffs[1] })
