<script lang="ts">
			...
			arg1: {
				...
				layout:
					| {
							...
							plot_bgcolor: string;
							xaxis: { title: string; showgrid: boolean; gridwidth: number };
							yaxis: { title: string; showgrid: boolean; gridwidth: number };
							title: string;
					  }
					| {
				...
				config: { responsive: boolean; scrollZoom: boolean };
			}
		) => any;
	};
	...
	getDataPromise
		.then((data: { workflow: any; dryrun: any }) => {
	...
	const drawNetworkReceivePlot = () => {
		...
		if (networkReceiveData.length === 0) {
			...
			let p3 = new Plotly.newPlot(plot3, {
	...
	const drawNetworkTransmitPlot = () => {
		...
		if (networkTransmitData.length === 0) {
			...
			let p3 = new Plotly.newPlot(plot4, {
	...
	<div class="table-container p-5">
		...
		{:then metricsData}
			...
			<div class="flex flex-row">
				...
				<div class="card p-4 basis-1/3">
					<div id="networkReceivePlot" />
				</div>
				<div class="card p-4 basis-1/3">
					<div id="networkTransmitPlot" />
				</div>
			</div>
		{/await}
	</div>
