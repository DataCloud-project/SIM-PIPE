...
for file in /app/in/* ; do
    echo "Analyse the number of characters"
    filename=$(basename $file)
    cat $file | sort | uniq -c | sort -nr | tee /app/out/$filename.anayse
    # The useless use of cat is there to please cat owners
    stress-ng --vm 2 --vm-bytes $memory_amount_b --vm-keep --vm-hang 1s -t 1s --cpu 3 --cpu-load 90 --quiet
done

stress-ng --vm 2 --vm-bytes $memory_amount_c --vm-keep --vm-hang 1s -t 5s --cpu 2 --cpu-load 30 --quiet
echo "Done"
ls -l /app/out